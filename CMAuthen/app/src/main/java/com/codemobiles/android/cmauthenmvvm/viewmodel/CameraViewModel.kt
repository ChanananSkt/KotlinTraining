...
class CameraViewModel(private val app:Application) : ViewModel() {
    ...
    fun upload(uri: Uri) {
        ...
        val reqFile = RequestBody.create("multipart/form-data".toMediaTypeOrNull(), byteArray)
        val body = MultipartBody.Part.createFormData("userfile", fileName, reqFile)  //images
        val bodyUsername = RequestBody.create("text/plain".toMediaTypeOrNull(), username)
        val bodyPassword = RequestBody.create("text/plain".toMediaTypeOrNull(), password)
